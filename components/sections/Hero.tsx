...
const WHATSAPP_MESSAGE = "Hola Suescun Signature Travel, deseo solicitar una cotización personalizada. Por favor me brindan información de planes, fechas y tarifas. ¡Quedo atento!";
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/60 backdrop-blur-md border-b border-[#C5A880]/15 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 h-20 flex items-center justify-between">
       ...
       <Link href="/" className="flex items-center gap-3 group">
        ...
        {/* Navegación Principal con las 4 pestañas solicitadas */}
        <nav className="hidden md:flex items-center gap-8">
          <a href="#inicio" className="text-xs uppercase tracking-[0.2em] text-[#C5A880] hover:text-white transition-colors">
            Inicio
          </a>
          <Link href="/coordenadas" className="text-xs uppercase tracking-[0.2em] text-neutral-400 hover:text-[#C5A880] transition-colors">
            Coordenadas
          </Link>
          <a href="#servicios" className="text-xs uppercase tracking-[0.2em] text-neutral-400 hover:text-[#C5A880] transition-colors">
            Servicios
          </a>
          <Link href="/contacto" className="text-xs uppercase tracking-[0.2em] text-neutral-400 hover:text-[#C5A880] transition-colors">
            Contacto
          </Link>
        </nav>

        {/* Botón Acción Encabezado (WhatsApp con mensaje personalizado) */}
...
function Hero() {
  return (
    <section id="inicio" className="relative w-full min-h-screen pt-28 pb-16 px-6 lg:px-12 max-w-7xl mx-auto flex items-center overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 w-full items-center z-10">
        ...
        <div className="lg:col-span-6 relative group">
          <div className="absolute -inset-0.5 bg-gradient-to-r from-[#C5A880]/30 to-transparent rounded-3xl blur opacity-30 group-hover:opacity-60 transition duration-1000"></div>
        ...
        <div className="lg:col-span-6 flex flex-col items-start gap-6 lg:pl-4">
          ...
          <h1 className="text-3xl sm:text-5xl lg:text-5xl font-serif text-white leading-[1.15] tracking-wide">
            ...
            <span className="italic font-normal bg-gradient-to-r from-[#E6D5B8] via-[#C5A880] to-[#9A7B4F] bg-clip-text text-transparent">
              coordenadas
            </span>{" "}
          ...
          <p className="text-neutral-300 text-sm sm:text-base font-light leading-relaxed max-w-xl">
            Diseño de viajes a la medida (<span className="text-white italic">&quot;Bespoke&quot;</span>), vuelos internacionales First Class y acceso exclusivo a las colecciones de hoteles más prestigiosas del planeta.
          </p>
          ...
          <div className="grid grid-cols-3 gap-6 pt-6 border-t border-neutral-800/80 w-full mt-2">
            <div>
              <p className="text-lg lg:text-xl font-serif text-[#C5A880]">100%</p>
              <p className="text-[10px] text-neutral-400 tracking-wider uppercase mt-0.5">A la medida</p>
            </div>
            <div>
              <p className="text-lg lg:text-xl font-serif text-[#C5A880]">24/7</p>
              <p className="text-[10px] text-neutral-400 tracking-wider uppercase mt-0.5">Atención VIP</p>
            </div>
            <div>
              <p className="text-lg lg:text-xl font-serif text-[#C5A880]">First Class</p>
              <p className="text-[10px] text-neutral-400 tracking-wider uppercase mt-0.5">Acceso Global</p>
            </div>
          </div>
...
function Pillars() {
  ...
  return (
    <section id="servicios" className="py-20 px-6 lg:px-12 max-w-7xl mx-auto border-t border-neutral-800/60">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
        <div>
          <span className="text-[10px] uppercase tracking-[0.25em] text-[#C5A880]">Exclusividad</span>
          <h2 className="text-2xl sm:text-4xl font-serif text-white mt-1">Servicios de Firma</h2>
        </div>
        <p className="text-neutral-400 text-xs sm:text-sm max-w-md font-light">
          Atención integral de punta a punta para que cada travesía refleje tus altos estándares de sofisticación.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {pillars.map((item, idx) => (
          ...
          >
            ...
            <h3 className="text-lg font-serif text-white mt-4 mb-2">{item.title}</h3>
...
function ContactCTA() {
  return (
    <section id="contacto" className="py-20 px-6 lg:px-12 max-w-7xl mx-auto">
      <div className="relative rounded-2xl border border-[#C5A880]/30 bg-gradient-to-b from-neutral-900/90 to-black p-8 sm:p-14 text-center overflow-hidden">
        {/* Destello de fondo */}
        ...
        <p className="text-neutral-300 text-xs sm:text-sm font-light max-w-lg mx-auto mb-8">
          Déjanos los detalles de tu próximo viaje o agenda una asesoría privada con nuestros especialistas en San Francisco o Bogotá.
        </p>

        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 max-w-md mx-auto">
          <Link
            href="/contacto"
            className="w-full sm:w-auto bg-gradient-to-r from-[#D4B891] via-[#C5A880] to-[#A88B60] text-black font-semibold text-xs tracking-[0.2em] uppercase px-8 py-4 rounded-sm transition-all hover:brightness-110 text-center"
          >
            Ir a Contacto
          </Link>
        </div>
